...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    let salesDocumentsIndex = -1;

    // Find the "Sales Documents" heading
    ...
    // Get all blocks after the "Sales Documents" heading until the next heading or end
    const documents = [];
    for (let i = salesDocumentsIndex + 1; i < blocksData.results.length; i++) {
      const block = blocksData.results[i];
