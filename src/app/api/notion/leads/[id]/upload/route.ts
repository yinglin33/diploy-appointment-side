...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // STEP 3: Find the "Sales Documents" heading and insert file after it
    ...
    let salesDocumentsHeadingId = null;
    let insertAfterBlockId = null;

    // Find the "Sales Documents" heading and determine where to insert
    for (let i = 0; i < blocksData.results.length; i++) {
      ...
      if (block.type === 'heading_2') {
        ...
        if (headingText.toLowerCase().includes('sales documents')) {
          salesDocumentsHeadingId = block.id;
    ...
    if (!salesDocumentsHeadingId) {
      return NextResponse.json({ 
        error: '"Sales Documents" section does not exist.' 
      }, { status: 404 });
    }
    ...
    // NOTE: inserting the file after the last file in the Sales Documents section (or after the heading if no files exist)
